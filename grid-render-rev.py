...
def moveToSquig(X, Y, DIR):	
    ...
    for i in range(int(dist)):
        ...
        freq = 0.25
        while a<af:
            xx = (x+width/2)/width*(img.width-af)+a
            yy = (y-yoff)/height*(img.height-af)
            coord = xx,yy
...
try:
    while col >= 0:
        ...
        if col%2==0:
            plot.moveTo(x,y1)
            moveToSquig(x,y2,1)
            plot.semiCirc(x-xsep/2,y2,xsep/2,two_pi*0.25, 3.14)
        else:
            plot.moveTo(x,y2)
            moveToSquig(x,y1,1)
            plot.semiCirc(x-xsep/2,y1,xsep/2,two_pi*0.25,-3.14)
        col-=1
   #plot.moveTo(0,0)
    print("end")
